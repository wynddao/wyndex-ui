...
const EditDistributionFlowModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    return (
      <>
        <Flex align="center" justify="space-between">
          ...
          <Button size="xs" colorScheme="red" onClick={() => handleRemoveElement(index)}>
            Remove
          </Button>
        </Flex>
        {!element.isCW20 ? (
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        ...
        <ModalCloseButton />
        <ModalBody>
          {elements.map((element, index) => formGroup(element, index))}
