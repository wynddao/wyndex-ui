                      ...
                      <Box key={i} mb={i === selectedVotes.length - 1 ? 0 : 3}>
                        <Flex justifyContent="space-between">
                          <Flex align="center">
                            ...
                            {vote.option.assets[0].hasOwnProperty("native") ? (
                              <span>
                                {/* @ts-ignore */}
                                {`${getAssetByDenom(vote.option.assets[0].native)?.symbol}`}
                              </span>
                            ) : (
                              // @ts-ignore
                              <TokenName symbol={true} address={vote.option.assets[0].token} />
                            )}
                            {" / "}
                            {vote.option.assets[1].hasOwnProperty("native") ? (
                              <span>
                                {/* @ts-ignore */}
                                {`${getAssetByDenom(vote.option.assets[1].native)?.symbol}`}
                              </span>
                            ) : (
                              // @ts-ignore
                              <TokenName symbol={true} address={vote.option.assets[1].token} />
                            )}
                          </Flex>
